...
-- Create more restrictive policies for Signup table (if needed at all)
-- Note: Consider if this table should be removed entirely
CREATE POLICY "Users can only insert their own signup record" 
ON public.signup 
FOR INSERT 
WITH CHECK (true); -- Allow signup creation for registration flow
...
-- Regular users cannot update or delete signup records
-- Only admins should have these permissions if needed
...
-- 5. Create audit trigger for role changes
...
BEGIN
  ...
  IF OLD.role IS DISTINCT FROM NEW.role THEN
    INSERT INTO public.signup (created_at) 
    VALUES (now()); -- Using signup table as simple audit log for now
    -- In production, you'd want a proper audit table
  END IF;
  RETURN NEW;
END;
$$;
